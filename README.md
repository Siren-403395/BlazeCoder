...
<div align="center"><sub>一条命令：构建 → 写入配置 → 把 <code>zephyrcode</code> 装上 PATH。然后在任意目录敲 <code>zephyrcode</code> 即可。</sub></div>
...
cd zephyrcode
./install.sh           # 构建 + 写入 ~/.zephyrcode/.env + 装 launcher 到 ~/.local/bin
...
> 默认模型是 `deepseek-v4-pro`。如果你的 key 调不了它，在 `~/.zephyrcode/.env` 里把 `DEEPSEEK_MODEL` 改成你能用的（如 `deepseek-chat`）。
...
配置读取顺序，从低到高：`~/.zephyrcode/.env`  <  `<当前目录>/.env`  <  真实环境变量。只有 `DEEPSEEK_API_KEY` 是必填，其余都有默认值。

<table>
<thead><tr><th>变量</th><th>默认</th><th>作用</th></tr></thead>
<tbody>
<tr><td><code>DEEPSEEK_API_KEY</code></td><td>（无）</td><td>API Key。留空则用离线 stub 模型</td></tr>
<tr><td><code>DEEPSEEK_MODEL</code></td><td><code>deepseek-v4-pro</code></td><td>模型名（改成你 key 能调的即可）</td></tr>
...
`agent-core` 里**没有任何** TUI / HTTP / DeepSeek 引用，一切都通过端口（`ModelGateway`、`Workspace`、`Sandbox`、`SessionStore`、`MemoryStore`、`Clock`、`Logger`）跨界，所以它能用内存假实现跑全套单测，也能在 CLI 里**进程内**运行，换模型只是换一个适配器。
...
新增内核能力：**新工具** = 在 `agent-core/src/tools/builtin/` 加一个 `Tool` 并注册；**新护栏** = 一个 `PreToolUse` / `PostToolUse` 钩子（不动循环）；**新模型** = 一个 `ModelGateway` 适配器。
