...
- [x] P0-perm-2    rewrite PermissionEngine: behavior-priority over layered rules + decisionReason   (deps: perm-1)
- [x] P0-perm-3    persisted layered settings store + PermissionUpdate pipeline via buildRuntime   (deps: perm-2)
...
- [x] P1-context-4 consecutive-failure circuit breaker + PROMPT_TOO_LONG head-truncation escape   (deps: context-2)
- [x] P1-context-5 harden summarization prompt (no-tools guard, analysis scratchpad, all-user-msgs, next-step)   (deps: context-4)
- [x] P1-perm-4    always-allow suggestions + [once/always-local/always-project/no] ask flow   (deps: perm-3)
- [x] P1-harness-3 between-turns steering queue + blocking Stop hook   (deps: harness-2, ext-1)
...
- [x] P2-context-7 (notes-as-summary capability done; model-maintained NOTES.md wiring = follow-up) optional live session-notes file as a zero-cost summary source   (deps: context-3)
...
- [x] P2-harness-6 reactive compaction on context-overflow rejection (retry once, gated)   (deps: harness-2, context-4)
- [x] P2-harness-7 (config snapshot + transitions reducer; literal step() repackaging not needed — loop is already a transitions machine) config/deps/state three-way split (loop as near-pure reducer)   (deps: harness-2, harness-3, harness-4)
- [x] P2-ext-5 (loader+apply done; /output-style runtime switch = thin TUI follow-up)     markdown output-style loader feeding buildSystemPrompt   (deps: prompts-2)
- [x] P2-ext-6     design-only: MCP as a ToolSource port (mcp__server__tool)   (deps: perm-1, tools-6)
