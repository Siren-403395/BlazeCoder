...
# the CLI still installs fine and `zephyrcode --gui` will build the GUI on first use.
say "Building the desktop GUI…"
if (cd "$REPO_DIR" && pnpm --filter @zephyrcode/desktop build >/dev/null 2>&1); then
  ok "Desktop GUI built (launch with 'zephyrcode --gui')."
else
  say "Desktop GUI build skipped — 'zephyrcode --gui' will build it on first use."
fi
...
# Exported so the CLI can locate the desktop package for 'zephyrcode --gui'.
export ZEPHYRCODE_REPO="$REPO_DIR"
if [ "\${1:-}" = "--update" ]; then
  ...
  pnpm install --silent && pnpm --filter @zephyrcode/cli build >/dev/null \\
    && { pnpm --filter @zephyrcode/desktop build >/dev/null 2>&1 || true; } \\
...
printf '   Desktop GUI: \033[1mzephyrcode --gui\033[0m\n'
